...
if not st.session_state.solved:
        ...
        if st.button("Submit"):
            ...
            if user < st.session_state.number:
                st.audio("audio/new-notification.mp3", loop=False, format="audio/mp3", autoplay=False)
                st.warning("Higher number please!")
            elif user > st.session_state.number:
                st.audio("audio/new-notification.mp3", loop=False, format="audio/mp3", autoplay=False)
                st.warning("Lower number please!")
            else:
                st.audio("audio/success-trumpets.mp3", loop=False, format="audio/mp3", autoplay=False)
