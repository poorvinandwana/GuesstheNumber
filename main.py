...
if not st.session_state.solved:
        ...
        if st.button("Submit"):
            ...
            if user < st.session_state.number:
                st.audio("new-notification", loop=False, format="audio/new-notification.mp3", autoplay=False)
                st.warning("Higher number please!")
            elif user > st.session_state.number:
                st.audio("new-notification", loop=False, format="audio/new-notification.mp3", autoplay=False)
                st.warning("Lower number please!")
            else:
                st.audio("success-trumpets", loop=False, format="audio/success-trumpets.mp3", autoplay=False)
